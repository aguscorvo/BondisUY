...
/*Opcion select */
$ds('.map_filters_accordion_content').on(
  'click',
  '.map_filters_accordion_counter',
  function () {
...
$ds('.head_search_bar').on('click', 'i', function () {
  ...
  oli.each(function (index) {
    if (
      !$ds(this).html().toUpperCase().includes($ds(oin).val().toUpperCase()) &&
      $ds(oin).val() != ''
    ) {
...
function searchOptions(id) {
  ...
  if (id == 1) {
    ...
    select =
      '<select class="form-control form-control-sm text-secondary" id="selectEmpresas"></select>';
    ...
    $ds("a[href='#ui-basic']").click();
    select =
      '<input type="text" class="form-control form-control-sm" id="inputLinea">';
    ...
    $ds("a[href='#ui-basic']").click();
    select =
      '<label for="inputCalleA">Calle</label><input type="text" class="form-control form-control-sm" id="inputCalleA">';
    select +=
      '<label for="inputCalleB">Esquina</label><input type="text" class="form-control form-control-sm" id="inputCalleB" disabled>';
    ...
    $ds("a[href='#ui-basic']").click();
    select =
      '<label for="inputCalleN">Calle</label><input type="text" class="form-control form-control-sm" id="inputCalleN">';
    select +=
      '<label for="inputNumero">N\u00FAmero</label><input type="number" class="form-control form-control-sm" id="inputNumero" disabled>';
    ...
    $ds("a[href='#ui-basic']").click();
    $ds(card_title).html('Ver paradas deshabilitadas');
    // $ds(card_subtitle).html("Informaci\u00F3n");
    $ds(card_subtitle).html('');
    var form_group = $ds(card).find('.form-group');
    var table = $ds('#selectTableLineas').children('table').get(0);
    ...
    var txttable =
      '<thead class="text-align: center;"><tr><th>L&iacute;nea</th><th>Detalle</th></tr></thead><tbody></tbody>';
    ...
    var card = $ds('#to_do_some');
    var button =
      '<button style="width: 100%" class="btn btn-secondary btn-fw" id="confNuevaParada">Confirmar Ubicaci\u00F3n</button>';
    ...
    $ds(buttonConf).on('click', function () {
      ...
      getRecorridoCercanosNuevaParada(
        [point32721['x'], point32721['y']],
        DISTANCIA_LINEAPARADA
      );
    });
    ...
    $ds("a[href='#ui-busstop']").click();
    select =
      '<input type="text" class="form-control form-control-sm" id="inputEditParada"><br>';
    var button =
      '<button style="width: 100%" class="btn btn-secondary btn-fw" id="confUpdParada">Modificar</button>';
    ...
    var updparada = '';
    var databody = $ds('#selectTableLineas').children('table').get(0);
    var tablaUPDParadasHorasLista = $ds('#UPDParadasHorasLista')
      .children('table')
      .get(0);
    ...
    var databodyasociada = $ds('#selectTableAsociadaLineas')
      .children('table')
      .get(0);
    ...
    $ds(databodyasociada).on('click', 'tr', function () {
      if ($ds(this).hasClass('selected')) {
        ...
        $ds('#updParadaLineaDescripcion').val(
          $ds(this).find('td').eq(0).html() +
            '-' +
            $ds(this).find('td').eq(1).html()
        );
    ...
    var databodycercana = $ds('#selectTableCercanaLineas')
      .children('table')
      .get(0);
    ...
    $ds(buttonViewHorario).on('click', function () {
      if (lineasParadasHorario.hasOwnProperty(updparadaLinea)) {
        ...
        var existeLineaenTabla = $ds(tablaBody).find(
          '[data-counter_id="' + updparadaLinea + '"]'
        );
        ...
        if (existeLineaenTabla.length == 0) {
          for (var h in lineaUPDHorarios) {
            txttablec =
              '<tr data-counter_id="' +
              updparadaLinea +
              '" data-counter_exist= "T" data-counter_eliminar="F" data-counter_hora="' +
              lineaUPDHorarios[h] +
              '"><td>' +
              updparada +
              '</td><td>' +
              updparadaLineaNombre +
              '</td><td>' +
              lineaUPDHorarios[h] +
              '</td></tr>';
    ...
    $ds('#updParadaHoraADD').on('click', function () {
      if ($ds('#updParadaHora').val().trim() != '') {
        txttablec =
          '<tr data-counter_id="' +
          updparadaLinea +
          '" data-counter_exist= "F"  data-counter_eliminar="F" data-counter_hora="' +
          $ds('#updParadaHora').val() +
          '"><td>' +
          updparada +
          '</td><td>' +
          updparadaLineaNombre +
    ...
    $ds('#buttonUPDParadaREST').on('click', function () {
      ...
      $ds('#UPDParadasHorasLista tr').each(function () {
        if (
          $ds(this).attr('data-counter_exist') == 'F' &&
          $ds(this).attr('data-counter_eliminar') == 'F'
        ) {
          var horario = {};
          ...
          $ds(this).attr('data-counter_exist') == 'T' &&
          $ds(this).attr('data-counter_eliminar') == 'T'
        ) {
          var horario = {};
          horario['linea'] = $ds(this).attr('data-counter_id');
          horario['parada'] = updparada;
      ...
      if (updParadaGeo) addHorarioLineaRecorridoGEOM(updparada);
      ...
      if (!updParadaERROR) {
        ...
        $ds('#general_error_icon').html(
          '<h1 ><i class="mdi mdi-information-outline display-3 text-info" ></i></h1>'
        );
        ...
        $ds('#general_error_icon').html(
          '<h1 ><i class="mdi mdi-alert-outline display-3 text-danger"></i></h1>'
        );
    ...
    var card = $ds('#to_do_some');
    var button =
      '<button style="width: 100%" class="btn btn-secondary btn-fw" id="confNuevaLinea">Confirmar L\u00EDnea</button>';
    ...
    $ds("a[href='#ui-busline']").click();
    select =
      '<input type="text" class="form-control form-control-sm" id="inputLinea"><br>';
    var button =
      '<button style="width: 100%" class="btn btn-secondary btn-fw" id="confUpdLinea">Modificar</button>';
    ...
    var card = $ds('#to_do_some');
    var button =
      '<button style="width: 100%" class="btn btn-secondary btn-fw" id="confZonaLinea">Confirmar zona</button>';
