...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "bondisuy-ear.ear");

        // Current directory is the root of the "bondisuy-web" project. Go up one level
        // and enter the "bondisuy-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../bondisuy-ejb/target/bondisuy-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "bondisuy-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/bondisuy-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "bondisuy-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
